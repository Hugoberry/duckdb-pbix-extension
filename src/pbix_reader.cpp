...
static unique_ptr<FunctionData> PbixBind(ClientContext &context, TableFunctionBindInput &input,
	...
	if (!db.GetRowIdInfo(result->table_name, result->row_id_info)) {
		...
		result->rows_per_group = optional_idx(122880); // Default value
	}
...
BindInfo PbixReadBindInfo(const optional_ptr<FunctionData> bind_data_p) {
...
PbixReadFunction::PbixReadFunction()
    ...
    get_bind_info = PbixReadBindInfo;  
