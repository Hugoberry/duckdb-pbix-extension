...
std::vector<int> VertipaqDecoder::readRLEBitPackedHybrid(std::istringstream& stream, int entries, int min_data_id, int bit_width)
{
    kaitai::kstream ks(&stream);
    column_data_idf_t column_data(&ks);
...
void VertipaqDecoder::processVertipaqData(ClientContext &context, const std::string &path,  VertipaqDetails &details, VertipaqFiles &vfiles)
{
    ...
    std::cout << "IDF metadata: " << vfiles[idf_metadata].m_cbOffsetHeader << " -> "<< vfiles[idf_metadata].Size << std::endl;
    std::cout << "IDF: " << vfiles[details.IDF].m_cbOffsetHeader << " -> "<< vfiles[details.IDF].Size << std::endl;
    std::cout << "Size of all_decompressed_data: " << all_decompressed_data.size() << std::endl;
    ...
    if (!details.Dictionary.empty())
    {
        ...
        if (dictionary.dictionary_type() == column_data_dictionary_t::DICTIONARY_TYPES_XM_TYPE_STRING) {
            ...
            while (std::getline(ss, token, '\0')) {  // assuming null-terminated strings in buffer
                hashtable[index++] = token;
            }

            std::cout << "Real Dictionary size: " << hashtable.size() << std::endl;
            // return hashtable;
            ...
            std::cout << "Dictionary size: " << str_values.size() << std::endl;
            // return str_values;
        }
        ks_d.close();
    }
    //read IDF
    std::string idf_stream(all_decompressed_data.begin() + vfiles[details.IDF].m_cbOffsetHeader, all_decompressed_data.begin() + vfiles[details.IDF].m_cbOffsetHeader + vfiles[details.IDF].Size);
    std::cout << "IDF stream size: " << idf_stream.size() << std::endl;
    std::istringstream is_idf(idf_stream);
    auto result = readRLEBitPackedHybrid(is_idf, idf_m.count_bit_packed, idf_m.min_data_id, idf_m.bit_width);
    ...
    std::cout << std::endl;
    std::cout << "IDF size: " << result.size() << std::endl;
